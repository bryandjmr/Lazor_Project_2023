        ...
        for i in range(len(raw_grid)):
            for j in range(len(raw_grid[i])):
                ...
                if raw_grid[i][j] == "o":
                    ...
                    self.fixed_blocks.append = ([(x, y),'x'])
                    continue
                else:
                    block = block(raw_grid[i][j])
                    self.grid[x][y] = block
                    self.fixed_blocks.append = ([(x, y), block)
    ...
    def lazor_solver(self):
        ...
        **Returns** (need to figure this out)
            ...
            value: *int or float*
                ffff
        """
        all_configurations = possible_config(len(self.fixed_blocks))
        
        for configuration in all_configurations:
            create_grid(configuration, goals)
    
    def possible_config(remaining_blocks, movable_config = [],configs_list = []):
        """
        This function finds all possible configurations.

        **Returns** (need to figure this out)

            value: *list*
                 nested list of all possible configurations. Each configuration
                 is saved as a list with each block speicifed as a list with 
                 its position and block type.
        """
        
        if remaining_blocks == 0:
            config = movable_config + fixed_blocks
            configs_list.append(config)
            return
        
        else:
            for i in range(len(self.raw_grid[0])):
                for j in range(len(self.raw_grid)):
                    if not any((i,j) == self.fixed_blocks[0] for block in self.fixed_blocks):
                        if not any((i,j) == movable_config[0] for block in movable_config):
               
                            # Place a block in the current cell
                            movable_config.append([(i,j), self.movable_blocks[remaining_blocks - 1]])
            
                            # Recursively generate configurations for the remaining blocks
                            possible_config(remaining_blocks - 1, movable_config, configs_list)
